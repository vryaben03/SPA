...
export default class extends AbstractView{
    ...
    async getHtml(){
        return `<link rel="stylesheet" href="static/css/signupform.css">
    ...
    if (localStorage.getItem(obj.name) != null){
        ...
        if (subscriber.password != "no"){
            document.getElementById('est').innerHTML= "Этот абонент уже администратор группы.";
        }
        else{
            subscriber.password = obj.password;
            if (subscriber.groups == undefined) subscriber.groups = "Администраторы";
            else subscriber.groups += ", Администраторы"; 
